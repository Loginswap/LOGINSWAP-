<p align="center"> <img src="logo.png" alt="LoginSwap Logo" width="200"/> </p># LoginSwap – DeFi Swaps Made Simple

LoginSwap is a community-driven decentralized exchange (DEX) designed for **simplicity**, **security**, and **rewards** for $LOGIN holders.
* [Official Roadmap](./Roadmap.md)
We make token swaps fast, affordable, and beginner-friendly while rewarding holders with automatic benefits.

## Key Features
- One-click token swaps
- Ultra-low fees & lightning-fast transactions
- Automatic rewards for $LOGIN holders
- High security focus for new users
- Built for both beginners and pros

## Token: $LOGIN
- Anti-inflation mechanism
- Holder rewards
- Pre-sale coming soon with **50% discount**!

## Important Links
- **Website:** [www.loginswap.xyz](https://www.loginswap.xyz)
- **Twitter (X):** [@Loginswap](https://x.com/Loginswap)
- **Telegram:** [t.me/loginswap](https://t.me/loginswap)
- **Discord:** [discord.gg/C2zvZvcZb](https://discord.gg/C2zvZvcZb)
- **YouTube:** [@LoginSwap](https://www.youtube.com/@LoginSwap)
- **Medium:** [@loginswap](https://medium.com/@loginswap)
  

## Documentation [Tokenomics](./Tokenomics.md) [Roadmap](./Roadmap.md) [Whitepaper](./whitepaper.md)
  
## License
MIT License

Join the community and let's build DeFi together!  
#DeFi #DEX #Crypto #Token #Web3 #BSC #Solana
